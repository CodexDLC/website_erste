...
## ✅ Definition of Done
*   [x] Создан `nginx/Dockerfile`.
*   [x] Обновлен `docker-compose.prod.yml`.
*   [x] Обновлен CI/CD пайплайн.
*   [ ] Успешный деплой на VPS (Ожидается).
*   [ ] Сайт открывается по HTTPS, статика грузится (Ожидается).
*   [ ] На VPS (в папке `/opt/pinlite`) нет папок `nginx/` и `frontend/`, но всё работает (Ожидается).
